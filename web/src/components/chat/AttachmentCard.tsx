...
function TablePreview({
  ...
  const text = useAttachmentText(treeId, nodeId, info.name, info.size, true);
  ...
  if (text.data === undefined) return <p className="attachment-note muted small">Loading…</p>;
  if (!parsed) {
    return (
  ...
  return (
    <DataTable
      ...
      fullFileUrl={attachmentUrl(treeId, nodeId, info.name, true)}
    />
  );
}
...
export function AttachmentCard({ treeId, nodeId, info }: CardProps) {
  const openAttachment = useOpenAttachment();
  const url = attachmentUrl(treeId, nodeId, info.name);
  const previewable = canPreview(info);
  const open = (asText = false) =>
    openAttachment({ treeId, nodeId, attachment: info, ...(asText ? { asText } : {}) });
  const tooLarge =
  ...
  return (
    <li className="attachment-card">
      ...
      {tooLarge && <p className="attachment-note muted small">Too large to preview</p>}
...
export function StreamingCard({ item, unsaved }: { item: StreamingAttachment; unsaved: boolean }) {
  const name =
    ...
    item.requestedName ??
    (item.origin === 'url' ? 'downloading…' : 'attachment');
  return (
        ...
        <span className={`attachment-status attachment-status-${item.status}`}>
          {item.status === 'saving'
            ...
            : item.status === 'ready'
              ? unsaved
                ? 'not saved'
                : 'ready · available when the answer is saved'
              : item.message}
        </span>
      </div>
    </li>
